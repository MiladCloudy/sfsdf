  ...
  class PaymentModel {
    ...
    public static function fetch_invoice_by_hash($invoiceHash){
      ...
      $record = $db->first("
        ...
        LEFT    OUTER JOIN  x_user
                        ON  x_invoice.user_id = x_user.user_id

        WHERE   x_invoice.hash = :invoice_hash
      ", array(
        'invoice_hash' => $invoiceHash,
      ));
    ...
    public static function fetch_pending_invoices($userId){
      ...
      $records = $db->query("
        ...
        WHERE   x_invoice.user_id = :user_id
            AND (x_transaction.payed IS NULL OR x_transaction.payed = :payed)
      ", array(
        'user_id' => $userId,
        'payed'   => 0,
      ));
    ...
    public static function found_payed_invoice_for_current_date($userId){
      ...
      $record = $db->first("
        ...
        WHERE   x_invoice.user_id = :user_id
            AND x_transaction.payed = :payed
            AND x_invoice.startDate < :startDate
            AND x_invoice.endDate > :endDate
      ", array(
        'user_id'   => $userId,
        'payed'     => 1,
        'startDate' => $date,
