...
class ProductModel {
    ...
    public static function fetch_orders_by_cartId($cartId){
      ...
      $orders = $db->query("
        ...
        LEFT    OUTER JOIN  pym_product
                        ON  pym_order.product_id = pym_product.product_id

        WHERE   pym_order.cart_id = :cart_id
      ", array(
        'cart_id' => $cartId,
      ));
    ...
    public static function get_orders_count_by_cartId($cartId) {
      $db = Db::getInstance();
      $total = $db->first("
        SELECT  COUNT(pym_order.order_id) AS total
        FROM    pym_order

        LEFT    OUTER JOIN  pym_cart
                        ON  pym_order.cart_id = pym_cart.cart_id
    ...
    public static function fetch_openCart_by_userId($userId) {
      ...
      $cart = $db->first("
        ...
        WHERE   (payed IS NULL OR payed != 1)
            AND user_id = :user_id
      ", array(
        'user_id' => $userId,
      ));
