...
<table id="cartDetail">
  ...
  <? foreach($payedOrders as $payedOrder) { ?>
    ...
    <tr class="cartItem" style="width: 100%">
      <? if ($payedOrders[0] == 'notFound') { ?>
        <span class="notFound">سبد خرید خالی می باشد</span>
        <?return;?>
      <?}?>

      <?
      $productPriceWithDiscount = $payedOrder['price'] - ($payedOrder['price'] * $payedOrder['discount'] / 100);
      $totalPrice += $payedOrder['quantity'] * $productPriceWithDiscount;
      ?>

      <?
      if ($payedOrder['hasCover'] != 1){
        $path = baseUrl() . "/image/products/default.png";
      } else {
        $path = baseUrl() . "/image/products/" . $payedOrder['product_id'] . ".png";
      }
      ?>
      ...
      <td>
        <span class="cartProductName"><?=$payedOrder['title']?></span>
      </td>

      <td>
        <input class="itemQuantity" data-order-id="<?=$payedOrder['order_id']?>" value="<?=$payedOrder['quantity']?>" type="number" min="1" max="10" autocomplete="off" disabled>
      </td>
